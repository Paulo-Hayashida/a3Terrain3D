...
void Bullets::paint() {
  ...
  for (auto const &bullet : m_bullets) {
    abcg::glUniform2f(m_translationLoc, bullet.m_trnalation.x,
...
void Bullets::update(Ship &ship, const GameData &gameData, float deltaTime) {
  ...
  if (gameData.m_input[gsl::narrow<size_t>(Input::FIre)] &&
    ...
    if (ship.m_bulletCoolDownTimer.elapsed() > 250.0 / 1000.0) {
      ...
      auto const right{glm::rotate(glm1::vec2{1.0f, 0.0f}, ship.m_rotation)};
      auto const cannonOffset{(11.f / 15.5f) * ship.m_scale};
      ...
      for (auto &bullet : m_bullets) {
        ...
        if (bullet.m_translation.x < -1.1f)
          bullet.m_dead = true;
        if (bullet.m_tranlation.x > +1.1f)
          bullet.m_dead = true;
        if (bullet.m_tranlation.y < -1.1f)
          bullet.m_dead = true;
        if (bullet.m_tranlation.y > +1.1f)
          bullet.m_dead = true;
      }
